...
export class createCalendar {
  ...
  constructor() {
    ...
    this.currentMonth = this.createMonth(monthIndex+1 , currentYear);
  }
  ...
  public createMonth(monthNumber: number, year: number): Month {
    let month = new Month();
    month.title = this.months[monthNumber-1];
    month.monthNumber = monthNumber ;
    month.year = year;
    month.days = new Date(year, monthNumber, 0).getDate();  // this returns 31 or 30
